...
- Weather 有 1,512 行，即 9 个地点乘以 168 小时；没有缺失值、重复时间戳或非一小时
  间隔。Air Quality 有 1,080 行，即 9 个地点乘以 120 小时；时间序列同样连续。
- Air Quality 的 UV、PM2.5、PM10 各有 9 个缺失值，恰好是每个地点一个时间点，缺失率
  0.833%。这说明它是系统性的边界时段缺失，不宜当作随机缺失直接均值填补。
- Weather 请求了 9 个坐标，但 API 实际只返回 7 个不同的纬度网格；Air Quality 的实际
  网格更粗，只有 5 个不同纬度和 7 个不同经度。因此 Dandenong、Frankston、Ringwood
  的空气质量汇总完全相同并不一定代表真实环境相同，而可能是落入同一预测网格。
- 本次天气均温约在 10.69–12.20°C 之间。最高降水概率达到 100%，最大阵风达到
  77.4 km/h；推荐系统不能只看全期平均值，应匹配用户选择的具体小时。
- 温度与体感温度相关系数为 0.936，风速与阵风为 0.926。两组变量高度相关，但含义不同，
  在给家长解释推荐原因时可以选更易理解的指标，模型内部仍可保留全部字段。
- PM2.5 与 PM10 的相关系数为 0.964，说明这次快照中二者变化非常接近。UV 与颗粒物呈
